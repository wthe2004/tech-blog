...
$$\hat{A}_t = \delta_t + (\gamma\lambda)\delta_{t+1} + \dots + (\gamma\lambda)^{T-t-1}\delta_{T-1} \quad (11)$$
  ...
  * $\lambda$ (lambda) 是**时序差分误差**，用于在**偏差 (bias)** 和 **方差 (variance)** 之间做权衡。
      ...
      * 当 $\lambda=1$ 时（对应公式10的情况），退化成蒙特卡洛估计（不自举）。这种情况过度依赖采样的“运气”，即方差过高。关于GAE的详细讨论见[[10 - Areas/20 - Concepts/Generalized advantage estimation (GAE) 广义优势估计]]
...
[[10 - Areas/20 - Concepts/Explained Variance 解释方差]]
```python
