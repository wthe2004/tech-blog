...
**应用**：L2范数在科学和工程中无处不在。在机器学习中，它被用于[[10 - Areas/20 - Concepts/L2正则化]]（又称权重衰减），通过惩罚数值较大的模型参数来防止过拟合。它也是衡量误差（如均方根误差）的基础。
...
| **[[10 - Areas/20 - Concepts/L2范数]]** | $\sqrt{\sum_{i=1}^{n} x_i^2}$ | 欧几里得距离、向量的模 | [[10 - Areas/20 - Concepts/L2正则化]]、[[10 - Areas/20 - Concepts/最小二乘法]]、距离计算 |
...
有一种叫[[10 - Areas/20 - Concepts/L0范数]]的东西，定义为向量中非零元素的个数，但它其实不是范数，因为它不符合Absolute homogeneity.