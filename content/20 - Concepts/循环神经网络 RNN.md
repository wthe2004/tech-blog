...
**注意：Vanilla RNN很难解决[[20 - Concepts/梯度消失]]问题**（只能通过特殊的初始化/[[20 - Concepts/层归一化 layer normalization]]/[[20 - Concepts/循环归一化 Recurrent Normalization]]来缓解）
...
* **前向传播**:
    ...
    * 计算并累加[[20 - Concepts/交叉熵损失 Cross-Entropy Loss]]`loss`。$L_t = -\log(p_{t, \text{target}})$
...
* **反向传播**:
    ...
    * `np.clip(dparam, -5, 5, out=dparam)`: **梯度裁剪**，用于防止**[[20 - Concepts/梯度爆炸]]**问题。
...
```
* **[[20 - Concepts/Adagrad]] 优化器**: 脚本使用一个手搓的[[20 - Concepts/Adagrad]]来更新模型参数。`mem`变量用于累积梯度的平方，从而实现自适应的学习率。
